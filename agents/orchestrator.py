...
class MultiAgentOrchestrator:
    ...
    def __init__(self, quarantine_dir: str=None, require_human_approval:
        bool=None):
        ...
        self.require_human_approval = (require_human_approval if
            require_human_approval is not None else Config.
            REQUIRE_HUMAN_APPROVAL)
        self.workflow = self._build_workflow()
        print(f'✓ Multi-Agent Orchestrator initialized')
        print(
            f"  - Human-in-the-loop: {'ENABLED' if self.require_human_approval else 'DISABLED'}"
            )
    ...
    def _add_message(self, state: AgentState, from_agent: str, to_agent:
        str, message: str):
        msg = {'from_agent': from_agent, 'to_agent': to_agent, 'message':
    ...
    def _detect_node(self, state: AgentState) ->AgentState:
        ...
        if result.get('is_suspicious'):
            self._add_message(state, self.detection_agent.name, self.
                intelligence_agent.name,
    ...
    def _intelligence_node(self, state: AgentState) ->AgentState:
        print(
            f'🌐 [{self.intelligence_agent.name}] Querying threat intelligence...'
            )
        ...
        if result.get('is_known_malware'):
            ...
            self._add_message(state, self.intelligence_agent.name, self.
                analyst_agent.name,
    ...
    def _analyze_node(self, state: AgentState) ->AgentState:
        ...
        data = {'detection': state['detection_result'], 'intelligence':
            state['intelligence_result'], 'response': {}}
        result = self.analyst_agent.analyze(data)
        state['analysis_result'] = result
        severity = result.get('severity', 'Medium')
        self._add_message(state, self.analyst_agent.name, self.
            coordinator_agent.name, f'Analysis complete - Severity: {severity}'
            )
        return state
    ...
    def _coordinate_node(self, state: AgentState) ->AgentState:
        ...
        self._add_message(state, self.coordinator_agent.name,
            'HumanOperator' if self.require_human_approval else self.
            response_agent.name,
    ...
    def _human_approval_node(self, state: AgentState) ->AgentState:
        ...
        if not self.require_human_approval or not requires_approval:
            state['human_decision'] = {'approved': True, 'action':
                coordination.get('recommended_action', 'monitor'), 'reason':
                'Auto-approved (no human approval required)'}
            return state
        ...
        while True:
            ...
            if choice == '1':
                state['human_decision'] = {'approved': False, 'action':
                    'allow', 'reason': 'User approved - false positive'}
                print('✓ Process/file ALLOWED by user')
                self._add_message(state, 'HumanOperator', self.
                    response_agent.name,
                    'User chose to ALLOW. Treating as false positive.')
                break
            elif choice == '2':
                state['human_decision'] = {'approved': True, 'action':
                    'terminate_temporary', 'reason':
                    'User requested temporary termination'}
                print('✓ Process will be TERMINATED (temporary)')
                self._add_message(state, 'HumanOperator', self.
                    response_agent.name, 'User chose TEMPORARY termination.')
                break
            elif choice == '3':
                state['human_decision'] = {'approved': True, 'action':
                    'terminate_permanent', 'reason':
                    'User requested permanent termination and quarantine'}
                print(
                    '✓ Process will be TERMINATED and QUARANTINED (permanent)')
                self._add_message(state, 'HumanOperator', self.
                    response_agent.name,
                ...
                state['human_decision'] = {'approved': False, 'action':
                    'monitor', 'reason': 'User chose to monitor without action'
                    }
    ...
    def _respond_node(self, state: AgentState) ->AgentState:
        ...
        action = human_decision.get('action') or state['coordination_result'
            ].get('recommended_action', 'monitor')
        data = {'action': action, 'target': state['target']}
        result = self.response_agent.analyze(data)
        state['response_result'] = result
        state['final_action'] = action
        success = result.get('success', False)
        message = result.get('message', '')
        self._add_message(state, self.response_agent.name, 'System',
            f"Action '{action}' {'completed' if success else 'failed'}: {message}"
            )
        return state

    def process_event(self, event_type: str, target: Dict[str, Any]) ->Dict[
        str, Any]:
        initial_state = {'event_type': event_type, 'target': target,
            'detection_result': {}, 'intelligence_result': {},
            'analysis_result': {}, 'coordination_result': {},
            'response_result': {}, 'final_action': 'monitor', 'messages': [
            ], 'human_decision': {}, 'workflow_start_time': time.time(),
        ...
        execution_time = final_state['workflow_end_time'] - final_state[
            'workflow_start_time']
        print(f"\n{'=' * 70}")
        ...
        if final_state.get('human_decision'):
            print(
                f"   Human Decision: {final_state['human_decision'].get('reason', 'N/A')}"
                )
        print(f"{'=' * 70}\n")
        return final_state
