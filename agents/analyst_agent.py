"""
Analyst Agent
Uses AI (LLM) to analyze threats and provide natural language explanations.
"""
...
class AnalystAgent(BaseAgent):
    """Agent responsible for AI-powered threat analysis"""
    
    def __init__(self):
        super().__init__(name="AnalystAgent", role="Security Analyst (AI-Powered)")
    ...
    def analyze(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            data: Incident data (detection, intelligence, response)
    ...
    def analyze_incident(
        ...
        """
        Synthesize analysis from all agents using AI
        
        Args:
            detection: Detection agent results
            intelligence: Intelligence agent results
            response: Response agent results
        ...
        # Determine severity (with threat level fallback)
        ...
        user_explanation = self.llm_tools.explain_to_user(detection, intelligence, ai_summary)
    ...
    def _fallback_analysis(self, detection: Dict, intelligence: Dict) -> str:
        ...
        if is_known_malware:
            return (
                f"CRITICAL THREAT: This file is confirmed malware with {vt_malicious} "
                f"VirusTotal detections. Immediate quarantine recommended. "
                f"This is a known malicious file that poses significant risk to system security."
            )
        elif threat_level >= 7:
            return (
                f"HIGH THREAT: Multiple suspicious indicators detected (threat level: {threat_level}/10). "
                f"Reasons: {', '.join(reasons)}. "
                f"Recommend immediate termination and quarantine to prevent potential system compromise."
            )
        elif is_suspicious:
            return (
                f"MEDIUM THREAT: Suspicious behavior detected (threat level: {threat_level}/10). "
                f"Reasons: {', '.join(reasons)}. "
                f"Recommend monitoring or temporary termination. May be false positive."
            )
        else:
            return (
                f"LOW THREAT: No significant threats detected. "
                f"Process/file appears to be legitimate. Continue monitoring for any changes."
            )
    ...
    def _analyze_with_dual_llm(self, detection: Dict, intelligence: Dict) -> str:
        """Analyze using dual LLM system"""
        try:
...
Provide brief analysis: threat type, risk level, recommended action."""
            ...
            return response if response else self._fallback_analysis(detection, intelligence)
        except:
