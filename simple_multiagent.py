...
class SimpleMultiAgent:
    ...
    def analyze_process(self, proc_name: str, path: str, cmdline: str, pid: int
        ...
        self._send_message('DetectionAgent', 'IntelligenceAgent',
            f"Suspicious process found: {proc_name} (Threat: {detection_result['threat_level']}/10)"
            )
        intelligence_result = self._intelligence_agent(path)
        self._send_message('IntelligenceAgent', 'AnalystAgent',
            f"Threat score: {intelligence_result['threat_score']}/10. Known malware: {intelligence_result['is_known_malware']}"
            )
        analysis_result = self._ai_analysis(detection_result,
            intelligence_result)
        self._send_message('AnalystAgent', 'CoordinatorAgent',
            f"AI analysis complete. Severity: {analysis_result['severity']}")
        final_action = self._coordinator_agent(detection_result,
            intelligence_result, analysis_result)
        self._send_message('CoordinatorAgent', 'System',
            f'Final action decided: {final_action}')
        return {'final_action': final_action, 'detection': detection_result,
            'intelligence': intelligence_result, 'analysis': analysis_result}

    def _detection_agent(self, proc_name: str, path: str, cmdline: str, pid:
        int) ->Dict[str, Any]:
        threat_level = self._calculate_threat_level(proc_name, path, cmdline)
        reasons = self._get_threat_reasons(proc_name, path, cmdline)
        return {'threat_level': threat_level, 'reasons': reasons,
            'is_suspicious': threat_level >= 3}

    def _calculate_threat_level(self, proc_name: str, path: str, cmdline: str
        ) ->int:
        score = 0
        ...
        if 'downloads' in path.lower():
            score += 2
        if proc_name.lower() in ['encryptor.exe', 'locker.exe', 'crypt.exe',
            'ransomware.exe']:
            score += 8
    ...
    def _get_threat_reasons(self, proc_name: str, path: str, cmdline: str
        ) ->list:
        reasons = []
        if 'temp' in path.lower():
            reasons.append('Runs from Temp folder')
        if 'downloads' in path.lower():
            reasons.append('Runs from Downloads folder')
        if proc_name.lower() in ['ransomware.exe']:
            reasons.append('Name suggests ransomware')
        if 'powershell' in proc_name.lower() and '-enc' in cmdline.lower():
            reasons.append('Uses encoded PowerShell')
        return reasons
    ...
    def _intelligence_agent(self, path: str) ->Dict[str, Any]:
        try:
            ...
            if file_hash:
                ...
                return {'threat_score': score, 'is_known_malware':
    ...
    def _ai_analysis(self, detection: Dict, intelligence: Dict) ->Dict:
        ...
        if self.gemini_llm:
            try:
                prompt = (
                    f"Analyze security threat. Threat level: {threat_level}/10. Reasons: {', '.join(reasons)}. Provide brief analysis and severity (Low, Medium, High, Critical)."
                    )
    ...
    def _fallback_analysis(self, detection: Dict, intelligence: Dict) ->Dict:
        ...
        if threat_level >= 8:
            return {'summary':
                f"Critical threat detected. {', '.join(reasons[:2])}. Immediate termination recommended."
                , 'severity': 'Critical'}
        elif threat_level >= 5:
            return {'summary':
                f"Suspicious activity. {', '.join(reasons[:2])}. Recommend termination."
                , 'severity': 'High'}
        elif threat_level >= 3:
            return {'summary':
                f"Low-level suspicious behavior. {', '.join(reasons[:1])}. Monitor recommended."
                , 'severity': 'Medium'}
        else:
            return {'summary': 'Minimal threat. Process appears safe.',
                'severity': 'Low'}
    ...
    def _coordinator_agent(self, detection: Dict, intelligence: Dict,
        analysis: Dict) ->str:
        threat_level = detection.get('threat_level', 0)
...
if __name__ == '__main__':
    ...
    result = agent.analyze_process('ransomware.exe',
        'C:\\Users\\test\\AppData\\Local\\Temp\\ransomware.exe', '', 1234)
    print('\n--- FINAL RESULT ---')
